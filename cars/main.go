...
func (c *Cars) BerechneVerbrauch(ortschaft, ueberland float64) {
    c.Verbrauch = (ortschaft + ueberland) / 2.0
}
...
auto2.BerechneVerbrauch(16.5,10.0)
...
fmt.Printf("Marke: %s\nModel: %s\nBaujahr: %d\nPS: %d\nFarbe: %s\nVerkaufspreis: %.2f EUR\nSitze: %d\nTüren: %d\nKlimaanlage: %s\nVerbrauch: %.2f l/100km\nTraumfahrzeug: %t\n",
    ...
    // Berechnung des Verbrauchs
    auto1.BerechneVerbrauch(17.7, 8.9)
